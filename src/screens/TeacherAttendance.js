...
const TeacherAttendance = ({ navigation }) => {
  ...
  async function getdata(x) {
    ...
    setNoData(false);
    await fetch(
      `https://soop.io/api/v1/school_owner/teacher_attendances?id=${myid}&date=${x}`,
      {
        method: "GET",
      }
    )
      ...
      .then((responseJson) => {
        if (responseJson.status === 200) {
          console.log(responseJson.data.overall_data.total_percentage);
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >{`${item.present} out of ${item.total_teachers} present`}</Text>
                </View>
                <Text
                  style={{
                    fontSize: 20,
